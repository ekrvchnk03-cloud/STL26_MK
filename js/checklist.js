...
function renderChecklist() {
  ...
  for (const cat in categories) {
    ...
    categories[cat].forEach(item => {
      html += `
        <label class="checklist-row flex items-start gap-3 bg-slate-800 rounded-lg p-3 border border-slate-700 cursor-pointer hover:border-cyan-500/50 transition">
          <input type="checkbox" class="checklist-item mt-1 w-5 h-5 accent-cyan-500 flex-shrink-0" data-idx="${item.idx}" onchange="onCheck()">
          <span class="check-label text-sm text-slate-200">${item.text}</span>
        </label>
      `;
    });
    html += '</div>';
...
async function savePDF() {
  ...
  try {
    ...
    // Перед скриншотом заменяем чекбоксы на видимые иконки
    // потому что html2canvas не рисует настоящие checkbox
    const checkboxes = content.querySelectorAll('.checklist-item');
    const markers = [];
    ...
    checkboxes.forEach(cb => {
      ...
      if (cb.checked) {
        marker.textContent = '✅';
        marker.style.cssText = 'font-size:18px; margin-right:4px; flex-shrink:0;';
      } else {
        marker.textContent = '⬜';
        marker.style.cssText = 'font-size:18px; margin-right:4px; flex-shrink:0; opacity:0.5;';
      }
      cb.parentNode.insertBefore(marker, cb);
      cb.style.display = 'none';
      markers.push({ marker, cb });
    });
